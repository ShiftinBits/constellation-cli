...
export class TsJsConfigManager implements BuildConfigManager {
	...
	constructor(
		projectRoot: string,
		languages: IConstellationLanguageConfig
	) {
		this.projectRoot = projectRoot;
		this.isTypeScriptEnabled = 'typescript' in languages;
		this.isJavaScriptEnabled = 'javascript' in languages;
		this.isPathResolutionEnabled = this.isTypeScriptEnabled || this.isJavaScriptEnabled;
	}
	 ...
	 */
	async getConfigForFile(filePath: string): Promise<TSConfckParseResult | null> {
		...
		try {
			...
			const isTypeScriptFile = filePath.endsWith('.ts') || filePath.endsWith('.tsx') || filePath.endsWith('.d.ts');
			const isJavaScriptFile = filePath.endsWith('.js') || filePath.endsWith('.jsx') || filePath.endsWith('.mjs');
