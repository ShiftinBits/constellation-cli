...
describeIfGrammar('Python Vertical Slice Integration', () => {
	...
	describe('AST Serialization', () => {
		...
		it('should preserve string_content leaf text but strip compound string nodes', () => {
			const texts = collectTexts(serialized);

			// string_content leaf nodes now have text (needed for __all__ exports).
			// This means individual string values ARE visible as leaf text.
			expect(texts).toContain('do-not-leak-this-string');
			expect(texts).toContain('A helper that adds two numbers.');
			expect(texts).toContain('fetch failed');

			// Compound `string` nodes (Python) should still NOT have text set —
			// the parent string node is stripped, only the string_content child has text.
			...
			// `string_content` nodes SHOULD have text set (leaf text for __all__ exports)
			const stringContentNodes = findNodes(serialized, 'string_content');
			for (const scNode of stringContentNodes) {
				expect(scNode.text).toBeDefined();
			}
		});
