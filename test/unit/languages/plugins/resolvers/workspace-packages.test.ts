...
describe('TsJsImportResolver - Workspace Packages', () => {
	...
	it('should handle deep imports from workspace packages', async () => {
		...
		const result = await resolver.resolve(
			'@constellation/database/models/User',
		);
	...
	it('should resolve workspace packages with sub-paths', async () => {
		...
		expect(await resolver.resolve('@myorg/ui/Button')).toBe(
			'./libs/ui/src/Button.tsx',
		);

		// Sub-path (nested)
		expect(await resolver.resolve('@myorg/ui/components/Input')).toBe(
